...
Device requirements: modern browser with module workers, WebGL2 and `DecompressionStream`; HTTPS or localhost is needed for WebGPU and checksums. The app checks WebGPU adapter/device creation and model buffer limits before showing the loader or inspecting cached weights. Unsupported devices automatically start the labeled cooking demo. GPU kernel checks run before graph downloads; initialization failures or a stalled GPU switch to the demo instead of silently starting the large CPU model. Ordinary model-download failures retain retry controls. Allow several hundred MB for graph arrays, metadata and GPU buffers. Low-memory/mobile devices may struggle; desktop validation and a responsive viewport are not a guarantee of mobile hardware performance. No installation, API key, or trained-weight upload is required for visitors.
