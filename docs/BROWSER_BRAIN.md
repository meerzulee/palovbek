...
The default experience runs locally in the visitor’s browser. Cloudflare only serves files. Each visit gets a fresh session that begins cooking automatically after initialization. On page load, the app checks the actual Cache Storage keys against every pinned metadata/connection chunk hash. If any file is missing, a modal opens automatically; with a complete cache, a fresh brain initializes and starts cooking in the background without showing it. The worker still verifies the cached bytes before use. **Initialize brain** or **Load browser brain** reopens a dismissed modal; **Load weights · 79 MB** explicitly starts the download. The modal shows chunk verification, engine initialization, failures with retry, cancellation, and readiness. It can be closed to continue loading in the background. The modal closes automatically when initialization succeeds and Choyxona plov starts. A sticky toolbar above the kitchen provides pause/resume and another-cook controls, including on mobile. Selecting another recipe changes **Choose your plov** to **Cook this plov**, which starts that recipe immediately. Explicit resets and pauses do not trigger automatic restart. A blocked cache shows the first-visit loader, and download failures reopen it for retry; GPU failures start the cooking demo described below. There is no hosted Python requirement, model API, account, or training service.

## Browser compatibility and demo fallback

The page checks for a secure context, module-worker prerequisites, WebGPU adapter limits, and actual device creation before opening the weight loader or using a cached model. The probe is bounded to four seconds and destroys its temporary device, including late results. There is no Safari user-agent block: Safari 26 introduced WebGPU, but the exposed API alone does not guarantee this model can initialize. [WebKit announcement](https://webkit.org/blog/16993/news-from-wwdc25-web-technology-coming-this-fall-in-safari-26-beta/).

The worker runs its small GPU parity check before downloading the complete graph. Missing GPU support, incompatible kernels, full-model allocation failure, worker startup errors, or an unresponsive GPU select the automatically playing **Demo mode**. The neural worker is terminated, and the UI clearly labels the brain animation as illustrative. Download/network errors remain retryable in the loader, which also offers an immediate cooking-demo button. The CPU implementation remains available for explicit audits; it is no longer the browser UI's automatic fallback.

The demo has a pause/resume button in the sticky header. A manual mode choice cancels pending automatic startup, and switching EN/RU does not restart cooking. The fallback is selected per visit, so a later browser upgrade can be checked again.

Run `npx playwright test e2e/browser-support.spec.ts` for unsupported API, null adapter, insufficient limits, failed device, timeout, kernel failure, and touch controls. Install WebKit with `npx playwright install webkit` to include the WebKit viewport check. This tests the browser engine and mobile layout, not physical iPhone hardware.
