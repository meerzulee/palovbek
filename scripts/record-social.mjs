...
await page.addInitScript(() => {
  localStorage.setItem('plov-language-v1', 'en');
...
try {
  ...
  await page.addStyleTag({ content: `
    ...
    .minimal-main .neural-lab .kitchen-card { position:absolute!important; left:32px!important; top:272px!important; width:1016px!important; height:1000px!important; border-radius:28px!important; overflow:hidden!important; }
    ...
    .minimal-main .neural-dashboard { position:absolute!important; left:32px!important; top:1300px!important; width:1016px!important; height:342px!important; overflow:hidden!important; border-radius:28px!important; }
    ...
    #social-title { position:absolute; left:56px; top:42px; color:#b7ecd1; font:700 25px Arial; letter-spacing:5px; }
    #social-hook { position:absolute; left:56px; top:94px; color:#f4f5ef; font:700 66px/1.04 Arial; letter-spacing:-2px; }
  ...
  ` });
  await page.evaluate(() => {
    for (const [id, text] of [['social-title','PALOVBEK'], ['social-hook','166,700 neurons.\nOne qazan.'], ['social-step','Six legs. One very serious recipe.']]) {
    ...
    const note=document.createElement('span'); note.innerText='Recipe-guided cooking\nReal simulated spikes'; footer.append(note); document.body.append(footer);
    scrollTo(0,0);
  });
  ...
  while (Date.now() < deadline) {
    const state = await page.evaluate(() => {
      ...
      const labels=['Heat the oil. Build the flavour.', 'Onions, lamb, and a little patience.', 'Matchstick carrots. Six-legged precision.', 'Zirvak: the best things take time.', 'Rice goes in. Spoon takes a break.', 'Lid on. Sabr, aka.', 'Osh tayyor! Plov is ready.'];
    ...
    if(state.outcome && !doneAt) {
      ...
      doneAt=Date.now();
      await page.getByRole('button', { name:'Qazan cam', exact:true, includeHidden:true }).evaluate(button=>button.click());
    }
  ...
  await writeFile(resolve(output,'capture.json'),JSON.stringify({recorded_at:new Date().toISOString(),...metadata,errors,events},null,2)+'\n');
